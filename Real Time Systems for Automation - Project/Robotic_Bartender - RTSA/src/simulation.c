 ...
 *  
 *  CHANGED SOMETHING FOR SOME ISSUE (FIXING OF AN ERROR REGARDING THE DISPLAY ON THE TERMINAL)
 */
...
void StartSimulation(int sec) {
	...
	/* CurrentPriority[0][task]=TOP_PRIORITY+task; */ /* commented because we deleted HLP */
    }
...
void CleanUp() {
    ...
    logMsg("Response times: %d %d %d %d\n", 
        (_Vx_usr_arg_t) ResponseTime[0], (_Vx_usr_arg_t) ResponseTime[1], 
        (_Vx_usr_arg_t) ResponseTime[2], (_Vx_usr_arg_t) ResponseTime[3], 0, 0); /* Changed logMsg for error of too many arguments*/

    logMsg("Overruns: %d %d %d %d\n", 
        (_Vx_usr_arg_t) Overruns[0], (_Vx_usr_arg_t) Overruns[1], 
        (_Vx_usr_arg_t) Overruns[2], (_Vx_usr_arg_t) Overruns[3], 0, 0); /* Changed logMsg for error of too many arguments*/
    if (BatchMode)
        semGive(BatchSem);
}
